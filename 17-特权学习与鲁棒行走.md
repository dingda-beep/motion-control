...
若只收集 Teacher 访问过的状态，Student 一旦犯小错，就进入数据集外状态；在那里再犯更大错误，误差逐步积累。这叫 covariate shift（协变量偏移/状态分布偏移）。

这类似于只用生产系统的成功请求训练故障恢复模型：模型自己造成的新异常状态从未出现在训练集里。
...
![DreamWaQ 框架图](../图片和附件/image%209.png)

...
- **第一遍读第 1～4 节和第 7 节**：只理解“质心像倒立摆、ZMP 是脚底可移动的等效支撑点、身体冲得越快就要落脚越靠前”。
- **第二遍读第 5～6 节**：需要实现 LIP-MPC 时，再看离散矩阵和代价函数。
- `cosh`、`sinh` 的具体计算第一遍可以完全跳过；它们只是把连续方程精确推进一个时间步的数学工具。
...
先看方向，不急着算。若质心在支撑点 `p` 的前方，即 `x>p`，倒立摆会继续向前倒，因此 `x_ddot>0`；质心离支撑点越远，这个向前加速趋势越强。LIP 用下面的线性比例表达这件事。
...
平方 `ω²` 一定非负，所以加速度方向由 `(x-p)` 的正负决定。这个符号关系就是整条公式最值得第一遍记住的内容。
...
- `τ_x,τ_y`：足底传感器相对其测量原点得到的横滚/俯仰力矩；
- `F_z`：竖直法向力；
- 符号正负取决于坐标系约定。

因此算出的 `p_x,p_y` 也是**相对同一个测量原点**的位置，不是未经变换的世界坐标。若传感器原点不在脚底平面，还要根据传感器安装位姿修正力矩和位置。

`F_z` 接近零时不能使用这个比值，因为脚没有可靠接触，数值也会爆炸。

## 5. 离散模型

为避免让同一个 `x_k` 同时表示“状态向量”和“质心位置”，这里把二维状态向量记作 `s[k]`：
...
- `s[k]`：第 `k` 步的 LIP 状态向量；
- `x[k]`：第 `k` 步质心位置；
- `v_x[k]`：第 `k` 步质心水平速度，也就是连续公式里的 `x_dot`；
- `u[k]`：模型的控制输入；本章选择 ZMP 位置 `p[k]` 作为输入。
...
- `Cost`：预测窗口内所有阶段代价的总和；这里不用 `J`，避免和雅可比矩阵混淆；
- `Σ`：把第 0 步到第 `N-1` 步的括号项全部相加；
- `q_v`：速度误差权重；
- `r_p`：ZMP 偏差权重；
- `v_ref[k]`：第 `k` 步的参考质心速度；
- `p_ref[k]`：第 `k` 步偏好的 ZMP 位置；
- `N`：预测步数。
...
- `p_min[k]、p_max[k]`：第 `k` 步当前支撑区域允许的 ZMP 下界和上界；这里是一维写法，二维时应限制在支撑多边形内。
...
希腊字母 `ξ` 读作 xi，这里只是捕获点位置的名字。该公式建立在本章的固定质心高度、平地和忽略角动量等 LIP 假设下，不是任意真实动作的精确落脚公式。

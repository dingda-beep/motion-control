...
```text
# 导出不是“把 checkpoint 改个后缀”，还要读取训练环境最终解析出的接口事实。
policy.onnx, deploy_config, provenance = export(
    selected_checkpoint,            # 通过固定评估后选中的参数快照
    resolved_training_environment,   # 实际生效的观测、动作、周期和归一化配置
    robot_joint_definition,          # 关节名称、顺序、默认姿态与 SDK 映射
    software_versions                # 训练代码、框架、资源和导出器版本
)
# 三个输出分别回答：网络怎么算、两端数字什么意思、它从哪里产生。
```

...
本章贯穿一个具体场景：机器人正在向前走，未来 `0.4 s` 内会经历“左脚支撑 → 双脚支撑 → 右脚支撑”。此时身体又受到一次向左的推力。控制器需要提前决定：未来每个时刻，哪只脚应该给身体多大的三维接触力，才能既继续前进又把侧向运动接回来。
...
状态估计给出当前质心、姿态和速度
→ 接触计划说明未来每一步哪些脚能够用力
→ SRBD 预测不同接触力会怎样改变整体运动
→ 凸 MPC 在摩擦和单边接触限制内选择未来接触力序列
→ 只把第一步接触力参考交给 WBC
→ WBC 用完整全身模型生成当前关节力矩
→ 真实反馈进入下一周期重新预测
```

后面的每条方程都只是在展开这条链中的一环。若忘记某个符号，可以先回来确认：这一节是在描述“整体怎样动”“脚能怎样用力”，还是“高层结果怎样交给关节”。
...
牺牲关节细节，换来的是能够在有限计算时间内多看未来几步。这里的简化不是认为四肢不存在，而是认为当前预测任务最需要保留的是整机平移、整机转动和外部接触作用。
